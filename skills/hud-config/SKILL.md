...
1. **发起交互选择**：
   ...
   - **颜色主题**：深海青蓝 (`ocean`，默认科技风)、翡翠绿 (`emerald`，清新护眼薄荷绿)、赛博朋克 (`cyberpunk`，粉紫霓虹青撞色)、琥珀金 (`amber`，耀眼高亮金黄)、黑白极简 (`monochrome`，经典终端灰白)
   ...
   - **告警阈值**（置于根对象下，可选）：
     ...
     - `defaultEffortLevel`: 推理深度回退档位，默认 `"medium"`（可选 `"low"`, `"medium"`, `"high"`, `"xhigh"`, `"max"`, `"ultracode"`）
