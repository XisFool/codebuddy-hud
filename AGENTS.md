...
```bash
# 单元测试（scripts/run-tests.js 自动向 node --test 传参，兼容 Node 18~24+ 全平台）
npm test
...
- **验证闭环**：提交前必须在终端全量运行并通过 `npm test`、`npm run verify` 及 `npm run verify:install`；用例数量以当次实际输出为准，严禁带错提交。
