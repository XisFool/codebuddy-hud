...
宿主（实测至最新 v2.157.0）在会话、结果、配置等 5 大离散事件后约 300ms 去抖触发 HUD。空闲与只读工具执行时不周期刷新；命令失败会清空 HUD，且不会自动定时重试。
...
宿主（源码确认为最新 v2.157.0 常量）仅保留 stdout 前 3 行。HUD 的 3 行输出契约与该截断上限严格对齐；工具活动并入 Line 3，确保所有关键信息在截断限制下完整可见。
...
| :--- | :--- | :--- | :---: |
| **空 Stdin / 异常数据** | 宿主启动时序抖动 / 管道溢出 / 非法 JSON | 不输出任何行（0 字节）并安全退出；向错误日志写入一行带原因诊断，不伪造遥测 | `0` |
...
| **Git 超时 / 探测失败** | 庞大 Mono-repo / 非可用仓库目录 | 若能直接读出分支则保留分支并返回 `dirty: null`，否则隐藏；非可用仓库记录 60s 负缓存防重复派生 | `0` |
